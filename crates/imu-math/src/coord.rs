...
/// Mirrors C# `CoordinateUtility.ToQuaternion(double x, double y, double z)` which
...
/// Mirrors C# `CoordinateUtility.QuaternionToEuler` which converts to degrees at the end.
...
/// Mirrors C# `CoordinateUtility.QuaternionToEulerZXY`.
...
/// Mirrors C# `CoordinateUtility.GetYawFromQuaternion`.
...
/// Mirrors C# `CoordinateUtility.GetXAxisFromQuaternion`.
...
/// Apply rotation `q` to vector `v` — `v' = q · v · q⁻¹`. Mirrors C# `Transform`.
pub fn transform_v(v: Vector3<f32>, q: UnitQuaternion<f32>) -> Vector3<f32> {
    q.transform_vector(&v)
}

/// Build a unit quaternion that rotates `from` to `to`. Mirrors C#
...
mod tests {
    ...
    fn yaw_only_extraction() {
        // C# `GetYawFromQuaternion` extracts rotation about Y-axis directly (legacy
    ...
    fn pitch_only_extraction() {
        // C# `GetXAxisFromQuaternion` extracts X-axis rotation. euler_xyz_to_quat(30, 0, 0)
