...
/// Decompose a unit quaternion into (axis, angle in radians). Mirrors SlimeIMU `QuaternionUtils.ToAxisAngle`: axis fallback `(1, 0, 0)` if `s < 1e-4`.
...
/// Mirrors SlimeIMU `QuaternionUtils.LocalRotation`.
...
/// direction. Mirrors SlimeIMU `QuaternionUtils.QuatFromGravity` (used for JC1 SPI cal helper).
