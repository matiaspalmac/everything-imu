...
/// Packet IDs and layouts — confirmed against SlimeIMU v0.4.x `PacketBuilder.cs` +
...
#[non_exhaustive]
pub enum SbPacket {
    /// Heartbeat / keepalive. SlimeVR-Server expects one trailing `u8` tracker id (0).
    #[deku(id = "0")]
    ...
    Handshake {
        ...
        /// as a single byte. The reference encoder calls
        /// `WriteInt32((int)imuType)` here.
        #[deku(pad_bytes_before = "3")]
        imu: ImuType,
        mcu: McuType,
        /// 3 × i32 IMU info slots. Slot 0 carries [`MagnetometerStatus`] in the
        /// SlimeIMU v0.4.x implementation; slots 1 and 2 are reserved/zero.
        imu_info: (i32, i32, i32),
        /// SlimeVR firmware protocol version. SlimeIMU v0.4.x sends 19.
...
/// The board design for a SlimeVR tracker. Values match the SlimeIMU
...
pub enum BoardType {
    ...
    #[deku(id = "3")]
    NodeMCU,
    /// `BoardType::Custom = 4` — used by the SlimeIMU v0.4.x bridge to identify
...
/// SlimeIMU v0.4.x layout) and in [`SbPacket::SensorInfo`] (1-byte form).
...
/// OK — SlimeIMU v0.4.x always sends `1` after registration so the dashboard goes
/// green immediately.
#[derive(Debug, Clone, PartialEq, Eq, DekuRead, DekuWrite)]
#[deku(id_type = "u8", ctx = "_: deku::ctx::Endian", endian = "big")]
...
/// be interpreted. `Normal = 1` is the only value sent by SlimeIMU v0.4.x.
#[derive(Debug, Clone, PartialEq, Eq, DekuRead, DekuWrite)]
#[deku(id_type = "u8", ctx = "_: deku::ctx::Endian", endian = "big")]
