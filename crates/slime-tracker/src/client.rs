...
pub struct HandshakeInfo {
    ...
    pub imu: ImuType,
    pub mcu: McuType,
    /// `imu_info[0]` slot — used by C# v0.4.1 to carry magnetometer status
...
impl SlimeClient {
    ...
    /// Advertise tracker-side feature bits to the server. C# v0.4.1 sends
        ...
        match socket.recv(&mut buf).await {
            ...
            Err(e) => {
                ...
                if matches!(
                    ...
                    #[cfg(feature = "client")]
                    tracing::debug!(
                        "slime-tracker receive_loop transient {kind:?} (server probably down): {e}"
                    );
                    continue;
                }
                #[cfg(feature = "client")]
        ...
        if !handshake_confirmed.load(Ordering::Acquire) {
            match send_handshake_inner(&socket, &seq, &info).await {
                Ok(()) => {
                    ...
                    last_send_ms_unix.store(now_ms_unix(), Ordering::Release);
                    #[cfg(feature = "client")]
                    tracing::debug!("watchdog: re-sent handshake");
                }
                Err(e) => {
                    #[cfg(feature = "client")]
